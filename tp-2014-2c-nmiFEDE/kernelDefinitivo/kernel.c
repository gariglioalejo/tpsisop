...
bool compararIdNodo(void * nodo) {
	t_nodoRecurso * nodoAux = nodo;
	return nodoAux->idRecurso == idRecurso;
}
				...
				if (FD_ISSET(i, &readfds)) {
					...
					if (list_any_satisfy(listaSocketsCpu, comparar)) {
						...
						if ((recibido = recv(i, &unaOperacion, sizeof(int), 0))
							...
							case DESPERTAR: {
								idRecurso = recibirInt(i);
								t_nodoRecurso * nodoDesperto = list_find(
										listaBloqRecurso, compararIdNodo);
								t_tcb * tcbCpu = malloc(sizeof(t_tcb));
								tcbCpu = nodoDesperto->tcb;
								list_remove_by_condition(listaBloqRecurso,
										compararIdNodo);
								pthread_mutex_lock(&mutexReady);
								queue_push(colaReady, tcbCpu);
								sem_post(&hayEnReady);
