...
pthread_mutex_t mutexComparador;pthread_mutex_t mutexPrueba;
...
int main(void) {t_config * archivoConfiguracion;
	...
	log_info(archivoLogMSP,"Inicia la MSP. Memoria principal=%u KB. Tamaño SWAP=%u MB.",cantidadMemoria,cantidadSwap);
	gestionarMemoria();
	return 1;
}
...
pthread_mutex_init(&mutexColaMarcosLibres,NULL);pthread_mutex_init(&mutexPrueba,NULL);
...
while(flag){pthread_mutex_lock(&mutexPrueba);
...
if(i==2){int PID; uint32_t direccion;
		...
		log_info(archivoLogMSP,"Solicitud de destruir segmento PID:%u. Direccion:%u.",PID,direccion);
...
uint32_t destruirSegmento(uint32_t PID,uint32_t baseSegmento){
	...
	pthread_mutex_unlock(&mutexComparador);
	if (segmento==NULL){log_error(archivoLogMSP,"No existia el segmento:%u que se pedia destruir por el PID:%u",numSegmento,PID);return -1;};
	i=segmento->tamanio;
...
respuesta_t crearSegmento(uint32_t PID,uint32_t tamanio){
	...
	t_list * tablaSegmentos;
		if(tamanio>1048576){respuesta.exito=-1;log_error(archivoLogMSP,"Se pide crearSegmento con un tamaño mayor al maximo PID:%u",PID);return respuesta;}
		if (hayMemoria(tamanio)){
			...
			if(list_is_empty(direccionListaProcesos)){pthread_mutex_unlock(&mutexTablaProceso);
				...
				tablaSegmentos=crearProceso(PID);
								...
								log_info(archivoLogMSP,"Se creó un nuevo segmento para el PID:%u. Tamaño:%u",PID,tamanio);
								tamanioAux=tamanio;
								if (tamanio%256!=0){
			...
			if (proceso != NULL){
				...
				direccionBaseSegmento=ingresarSegmento(tablaSegmentos,tamanio);
				pthread_mutex_unlock(&mutexTablaSegmento);
				if(direccionBaseSegmento==NULL){respuesta.exito=0; log_error(archivoLogMSP,"No pudo crearse segento para PID:%u ya tiene n°max de segmentos",PID);return respuesta;}
				respuesta.exito=1;
				respuesta.direccion=direccionBaseSegmento;
				log_info(archivoLogMSP,"Se creó un nuevo segmento para el PID:%u. Tamaño:%u",PID,tamanio);
				tamanioAux=tamanio;
			...
			log_error(archivoLogMSP,"No habia suficiente memoria para crear segmento PID:%u",PID);
			respuesta.exito=0;
			return respuesta;
	}
