...
int extraerOffset(int);
int extraerPagina(int);
uint32_t imprimirTablaSegmentos();
uint32_t imprimirTablaPaginas();

int main(void) {t_config * archivoConfiguracion;//mete pid 1 seg 45, pid 3 seg54, pid24 seg 46,(todos tamaño 4mb)
 	archivoConfiguracion=config_create("archiconf/MSP.txt");
	puerto=config_get_int_value(archivoConfiguracion,"PUERTO");
...
void consola(){
	...
	if (!(strcmp(opcion,"MEMORIA.LIBRE")))
		{printf("Cantidad memoria libre: %u \n", cantidadMemoriaDisponible);}
...
respuesta_t solicitarMemoria(uint32_t PID,uint32_t direccion,uint32_t tamanio){
	int i;char* buffer;uint32_t posicionInicial;
	...
	respuesta_entra=entraYDirTablaPags(respuesta_tablaSegmentos.direccion,direccion,tamanio);
...
int escribirMemoria(uint32_t PID,uint32_t direccion,char* bytes,uint32_t tamanio){
	...
	uint32_t segmento; uint32_t pagina;char *dirTablaPaginas;
	respuesta_tablaSegmentos=obtenerTablaSegmentos(PID);
	if(!respuesta_tablaSegmentos.exito) {return -1;};//exit, no exite PID
	respuesta_entra=entraYDirTablaPags(respuesta_tablaSegmentos.direccion,direccion,tamanio);
...
uint32_t destruirSegmento(uint32_t PID,uint32_t baseSegmento){
	...
	cantDePags=segmento->tamanio/256;
	pthread_mutex_lock(&mutexLlevarAMemoria);
	...
	cantidadMemoriaDisponible=cantidadMemoriaDisponible+(segmento->tamanio);
...
uint32_t ingresarSegmento(t_list * tablaSegmentos,uint32_t tamanio){
		...
		nuevoSegmento=malloc(12);
		nuevoSegmento->numSegmento=list_size(tablaSegmentos);
...
void imprimirInformacionProceso(elem_tipoProceso*);
...
bool marcoMenor(elem_tipoColaAuxiliar*,elem_tipoColaAuxiliar*);
void listarMarcos(){int contador;int cantidadMarcos;int i;
	...
	elem_tipoColaAuxiliar* elementoDeCola;
...
bool marcoMenor(elem_tipoColaAuxiliar* marco,elem_tipoColaAuxiliar* marco2){
