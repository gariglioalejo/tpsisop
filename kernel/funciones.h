...
void convertirCola(t_list * listaMadre,t_list * listaTcb, int tipoLista) {
	...
	list_add_all(listaMadre, listaTcb);
}
...
if (codigo != -1) {
	...
	memcpy(mensaje_serializado + sizeof(codigo), &size_beso, sizeof(size_beso));
...
if (setsockopt(listenningSocket, SOL_SOCKET, SO_REUSEADDR, &yes, sizeof(int))
		== -1) {
